...
import { ExteriorCostItemFormData, ExteriorCostItem } from '@/lib/types';

interface ExteriorCostItemFormProps {
  initialData?: ExteriorCostItem;
  exteriorId: string;
  onSubmit: (data: ExteriorCostItemFormData) => Promise<void>;
  onCancel: () => void;
  isLoading?: boolean;
}

export function ExteriorCostItemForm({
  initialData,
  exteriorId,
  onSubmit,
  onCancel,
  isLoading,
}: ExteriorCostItemFormProps) {
  const [formData, setFormData] = useState<ExteriorCostItemFormData>({
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex items-center space-x-2">
        ...
        <Label htmlFor="isDefault" className="cursor-pointer">
          This is a Default (used when exterior is NOT selected)
        </Label>
      </div>
